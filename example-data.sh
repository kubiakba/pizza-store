...
#create products
product_id=$(curl --silent -d '{"type":"Kebab", "price":10.0, "description":"Small","kebabName":"Fet Kebab"}' -H "Content-Type: application/json" -X POST $host/products | awk -F ":||," '{print $4}' | awk '{print substr($0, 2, length($0) - 2)}')
curl -d '{"type":"PizzaTopping", "price":6.0, "toppingName":"salami"}' -H "Content-Type: application/json" -X POST $host/products
curl -d '{"type":"Pizza", "price":8.0, "size":"SMALL","dough":"THIN"}' -H "Content-Type: application/json" -X POST $host/products
